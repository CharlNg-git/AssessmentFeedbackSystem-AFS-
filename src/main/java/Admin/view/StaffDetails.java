...
public class StaffDetails extends JDialog {
    private FileManager fileManager = new FileManager();
    private ManageStaff parentPanel;
    private String staffID;
    private String mode; // "VIEW" or "EDIT"
    
    private JTextField txtName, txtIC, txtRace, txtAddress, txtContact, txtEmail, txtPass, txtJob, txtYear;
    ...
    public StaffDetails(Frame parent, ManageStaff parentPanel, String staffID, String mode) {
        super(parent, "Staff Details - " + mode, true);
    ...
    private JPanel createHeader() {
        ...
        JLabel title = new JLabel("  STAFF DETAILS (" + mode + ")");
    ...
    private JPanel createLeftPanel() {
        ...
        String[] nations = {"Malaysian", "International"};
        cmbNationality = new JComboBox<>(nations);
    ...
    private JPanel createRightPanel() {
        ...
        lblStaffID.setFont(new Font("Arial", Font.BOLD, 16));
        lblStaffID.setBorder(BorderFactory.createLineBorder(Color.BLACK));
        lblStaffID.setPreferredSize(new Dimension(250, 35));
        lblStaffID.setOpaque(true);
        lblStaffID.setBackground(new Color(240, 240, 240));
        addLabelAndComponent(rightPanel, gbc, 1, "Staff ID:", lblStaffID);
        
        addLabelAndField(rightPanel, gbc, 2, "Password:", txtPass = new JTextField());
        addLabelAndField(rightPanel, gbc, 3, "Job Title:", txtJob = new JTextField());
        addLabelAndField(rightPanel, gbc, 4, "Years Service:", txtYear = new JTextField());
    ...
    private void loadStaffData() {
        try (BufferedReader br = new BufferedReader(new FileReader(fileManager.getFilePath(DatabaseFile.STAFF.getFileName())))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                // Staff ID is Index 10
                if (data.length >= 11 && data[10].trim().equalsIgnoreCase(staffID)) {
                    ...
                    txtJob.setText(data[8].trim());
                    txtYear.setText(data[9].trim());
                    lblStaffID.setText(data[10].trim());
                    txtPass.setText(data[11].trim());
                    break;
                }
            }
        } catch (IOException e) { e.printStackTrace(); }
        
        txtEmail.setText(getEmailFromUserFile(staffID));
    }
    ...
    private void saveChanges() {
        ...
        if (!txtEmail.getText().trim().endsWith("@uni.edu09")) {
            JOptionPane.showMessageDialog(this, "Email must be a university email ending with '@uni.edu09'!", "Validation Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(fileManager.getFilePath(DatabaseFile.STAFF.getFileName())))) {
            ...
            while ((line = br.readLine()) != null) {
                String[] data = line.split(",");
                if (data.length >= 11 && data[10].trim().equalsIgnoreCase(staffID)) {
                    // Preserve extra fields like Role/Academic Leader if they exist
                    String existingRole = (data.length > 12) ? data[12] : "lecturer";
                    String existingAL = (data.length > 13) ? data[13] : "";
                    ...
                    String updated = String.join(",", 
                        ...
                        txtJob.getText().trim(), txtYear.getText().trim(), 
                        staffID, txtPass.getText().trim(), 
                        existingRole, existingAL 
                    );
                    lines.add(updated);
                } else { lines.add(line); }
            }
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(fileManager.getFilePath(DatabaseFile.USER.getFileName())))) {
            ...
            while ((line = br.readLine()) != null) {
                String[] data = line.split(",");
                if (data.length >= 4 && data[0].trim().equalsIgnoreCase(staffID)) {
    ...
    private void applyModeSettings() {
        if (mode.equalsIgnoreCase("VIEW")) {
            ...
            txtPass.setEditable(false); txtJob.setEditable(false); txtYear.setEditable(false);
    ...
    private void openDatePicker() {
        ...
        for (int i = 0; i < 50; i++) years[i] = String.valueOf(currentYear - 20 - i); 
