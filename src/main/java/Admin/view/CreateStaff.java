...
public class CreateStaff extends JDialog {
    ...
    private JTextField txtName, txtIC, txtRace, txtAddress, txtContact, txtPass;
    private JTextField txtJobTitle, txtYearService;
    private JComboBox<String> cmbGender, cmbNationality, cmbRole, cmbAcademicLeader;
    private JTextField txtDOB;
    private JLabel lblTpNumber;
    private Map<String, String> leaderMap;

    public CreateStaff(Frame parent, ManageStaff parentPanel) {
        super(parent, "Create New Staff", true);
    ...
    private JPanel createHeader() {
        ...
        JLabel title = new JLabel("  CREATE NEW STAFF");
    ...
    private JPanel createRightPanel() {
        ...
        JLabel sysTitle = new JLabel("JOB & SYSTEM INFO");
        ...
        addLabelAndField(rightPanel, gbc, 3, "Job Title:", txtJobTitle = new JTextField());
        addLabelAndField(rightPanel, gbc, 4, "Year of Service:", txtYearService = new JTextField());
        
        String[] roles = {"lecturer", "leader", "admin"};
        cmbRole = new JComboBox<>(roles);
        cmbRole.setBackground(Color.WHITE);
        addLabelAndComponent(rightPanel, gbc, 5, "Role:", cmbRole);
        
        cmbAcademicLeader = new JComboBox<>();
        cmbAcademicLeader.addItem("None");
        for (String leaderName : leaderMap.keySet()) {
            cmbAcademicLeader.addItem(leaderName);
        }
        cmbAcademicLeader.setBackground(Color.WHITE);
        addLabelAndComponent(rightPanel, gbc, 6, "Academic Leader:", cmbAcademicLeader);
    ...
    private void loadAcademicLeaders() {
    ...
    private void handleSave() {
        ...
        String selectedLeaderName = (String) cmbAcademicLeader.getSelectedItem();
        String selectedLeaderID = "";
        if (selectedLeaderName != null && !selectedLeaderName.equals("None")) {
            selectedLeaderID = leaderMap.get(selectedLeaderName);
        }
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(fileManager.getFilePath(DatabaseFile.STAFF.getFileName()), true))) {
            bw.newLine();
            bw.write(line);
            
            // Also write to user.txt
            String userLine = String.join(",", lblTpNumber.getText().trim(), (String) cmbRole.getSelectedItem(), txtName.getText().trim(), "", txtPass.getText().trim());
    ...
    private void openDatePicker() {
        ...
        for (int i = 0; i < 60; i++) years[i] = String.valueOf(currentYear - 22 - i);
